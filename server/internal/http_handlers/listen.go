...
func (h *handlers) Listen(writer http.ResponseWriter, request *http.Request){
	...
	for {
		...
		if part.FormName() == "data" {
			...
			//_, err = exec.Command("/usr/bin/php", "/Users/maxheckel/Sites/vinyl-analytics/server/server/test.php", fileName).Output()
			//if err != nil {
			//	log.Fatal(err)
			//}
			...
			err = json.Unmarshal([]byte(outString), listenResponse)
			if err != nil {
			...
			responses.WriteResponse(200, discogsAlbums[0], writer)
